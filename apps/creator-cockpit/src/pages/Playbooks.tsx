...
function ConversationFlowBuilder({
  ...
  const [selectedNodeId, setSelectedNodeId] = useState(session.flow.selectedNodeId ?? session.flow.nodes[0]?.id ?? null);
  ...
  const onNodesChange = useCallback((changes: NodeChange<FlowNode>[]) => setNodes((current) => applyNodeChanges(changes, current)), [setNodes]);
  const onEdgesChange = useCallback((changes: EdgeChange<FlowEdge>[]) => setEdges((current) => applyEdgeChanges(changes, current)), [setEdges]);
      ...
      addEdge(
        {
          ...
          markerEnd: { type: "arrowclosed", color: "rgba(34,211,238,.82)" }
        },
        current
      )
    );
  ...
  function addNode(type: ScriptVisualBuilderNodeType, position = { x: 180, y: 180 }) {
    const builderNode = createBuilderNode(type, position);
...
function Inspector({
  ...
  return (
    <aside className="min-h-0 overflow-y-auto border-l border-blue-500/18 bg-[#081524] p-4">
      ...
      {tab === "properties" ? (
        node ? <PropertiesPanel node={node} issues={nodeIssues} onUpdateNode={onUpdateNode} /> : <EmptyInspector>Select a node to edit its properties.</EmptyInspector>
      ) : null}
...
function PropertiesPanel({ node, issues, onUpdateNode }: { node: ScriptVisualBuilderNode; issues: FlowValidationIssue[]; onUpdateNode: (patch: Partial<ScriptVisualBuilderNode>) => void }) {
  ...
  return (
    <div>
      ...
      <Field label="Name">
        <input value={node.label} onChange={(event) => onUpdateNode({ label: event.target.value })} className="command-card w-full rounded-lg px-3 py-2 text-sm" />
      </Field>
...
function FlowNodeCard({ data, selected }: NodeProps<FlowNode>) {
  ...
  return (
    <div className={`w-[210px] rounded-lg border p-3 shadow-[0_18px_48px_rgba(0,0,0,.28)] ${selected ? "border-cyan-300 bg-[#15314E]" : "border-blue-500/20 bg-[#0D1B2A]"}`}>
      <div className="flex items-center gap-2">
        <Icon className="h-4 w-4 text-cyan-200" aria-hidden="true" />
        <div className="min-w-0 flex-1 truncate text-sm font-semibold text-white">{data.label}</div>
        {data.issues ? <span className="rounded-full bg-amber-400/16 px-2 py-0.5 text-[10px] font-semibold text-amber-100">{data.issues}</span> : null}
      </div>
      <div className="mt-2 truncate text-xs text-blue-100/58">{data.summary}</div>
    </div>
  );
}
...
function nodeSummary(node: ScriptVisualBuilderNode) {
  ...
  return stringValue(node.config.body) || stringValue(node.config.title) || "Configured node";
}
